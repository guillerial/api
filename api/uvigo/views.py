...
class SchedulesView(APIView):
    ...
    def put(self, request):
        ...
        if user_type == UvigoUser.TEACHER:
            ...
            if serializer.is_valid(raise_exception=True):
                try:
                    if user.id == Teacher.objects.get(teacherschedule=serializer.schedule).id:
...
class GroupsView(APIView):
    ...
    def put(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            group = Group.objects.get(code=serializer.validated_data['code'])
...
class Subjects(APIView):
    ...
    def get(self, request):
        distinct_subjects = Group.objects.raw('SELECT * FROM groups group by groups.subject_name')
        return Response(data=serializers.SubjectSerializer(instance=distinct_subjects, many=True).data)
